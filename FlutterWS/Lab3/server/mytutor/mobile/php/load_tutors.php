...
$sqlloadtutors = "SELECT tt.tutor_id, tt.tutor_email,tt.tutor_phone, tt.tutor_name, tt.tutor_password, tt.tutor_description, tt.tutor_datereg, GROUP_CONCAT(s.subject_name ORDER BY s.subject_id ASC) FROM tbl_tutors tt, tbl_subjects s WHERE tt.tutor_id=s.tutor_id AND tutor_name LIKE '%$search%' GROUP BY tt.tutor_id ASC";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()){
        ...
        $tutorlist['subjectname'] = $row['GROUP_CONCAT(s.subject_name ORDER BY s.subject_id ASC)'];
